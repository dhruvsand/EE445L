...
void fifo_push(int32_t data) {
	int32_t tempFifoEnd = (fifoEnd + 1) % size;
	if(tempFifoEnd == fifoStart) {
		return;
	}
	fifoEnd = tempFifoEnd;
	fifo[fifoEnd] = data;
...
int32_t fifo_CurrentValue(void) {
	return fifo[fifoStart];
}
int32_t* fifo_Get(void) {
	return fifo;// incase i want to analyze the entire array
}
